        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if nuevas_en_historial:
                    ejemplos = ", ".join(list(nuevas_en_historial.keys())[:5])
                    log(job, f"ℹ️ Aviso: {len(nuevas_en_historial)} de estas facturas ya aparecen en el historial de "
                              f"otra corrida anterior para esta IPS (ej: {ejemplos}). Se procesarán de todos modos; "
                              f"detén el proceso ahora si no era la intención.", "warn")
            except Exception:
                pass
...
def start_job():
    ...
    identidad = (data.get("identidad", "") or "").strip() or None
